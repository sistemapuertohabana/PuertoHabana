      ...
      <aside className="hidden lg:flex flex-col w-64 min-h-screen fixed left-0 top-0 bg-white border-r border-gray-200">
        ...
        <nav className="flex-1 mt-4 px-3 space-y-0.5">
          {menuItems.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link key={href} href={href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-150 ${active ? 'bg-cyan-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50'}`}>
