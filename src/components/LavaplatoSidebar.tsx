...
function navbarItem(s: NavbarStyle, active: boolean) {
  if (s === 'flotante')          return active ? 'bg-cyan-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50';
  ...
  return active ? 'bg-cyan-100 text-cyan-700' : 'text-gray-400 hover:bg-gray-100';
}
...
function sidebarItem(d: SidebarDesign, active: boolean) {
  ...
  if (d === 'bonito')      return active ? 'bg-cyan-600 text-white' : 'text-gray-600 hover:bg-cyan-50';
  if (d === 'azul')        return active ? 'bg-white text-cyan-600' : 'text-white hover:bg-blue-700';
  return active ? 'bg-cyan-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50';
}
...
export default function LavaplatoSidebar() {
  ...
  const navbar     = useLocalStorageValue('navbarStyle_lavaplato', 'original') as NavbarStyle;
      ...
      <nav className="lg:hidden fixed top-0 inset-x-0 z-50 border-b bg-white border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold leading-none text-gray-900">Lavaplatos</p>
        ...
        <div className={`p-5 border-b ${design === 'azul' ? 'border-blue-700' : 'border-gray-100'}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className={`text-sm font-semibold truncate ${design === 'azul' ? 'text-white' : 'text-gray-900'}`}>Lavaplatos</p>
