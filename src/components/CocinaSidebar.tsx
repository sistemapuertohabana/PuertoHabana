...
import { ChefHat, User, LogOut, Clock, Settings, DollarSign } from 'lucide-react';
      ...
      <aside className="hidden lg:flex flex-col w-64 min-h-screen fixed left-0 top-0 bg-white border-r border-gray-200">
        <div className="p-5 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-gray-900 truncate">Portal Cocina</p>
        ...
        <nav className="flex-1 mt-4 px-3 space-y-0.5">
          {menuItems.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link key={href} href={href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-150 ${active ? 'bg-orange-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-50'}`}>
        ...
        <div className="px-3 pb-4 border-t border-gray-100 pt-3">
          <button onClick={() => { localStorage.removeItem('ph_cocina_session'); window.location.href = '/'; }}
            className="flex items-center justify-center gap-2 w-full py-2 rounded-lg transition-colors text-sm font-medium text-red-500 hover:bg-red-50">
