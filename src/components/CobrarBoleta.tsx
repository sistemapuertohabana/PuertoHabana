...
export default function CobrarBoleta({
  ...
  const [mpPreferencia, setMpPreferencia] = useState<{ initPoint: string; comandaTotal: number } | null>(null);
    ...
    try {
      const res = await fetch('/api/mercadopago/create-preference', {
        ...
        body: JSON.stringify({
          ...
          total,
          mesa: mesaLabel,
          items: pedidos.map(p => ({ nombre: p.item, cantidad: p.cantidad, precio: p.precio })),
          origin: typeof window !== 'undefined' ? window.location.origin : '',
        }),
      });
      const data = await res.json();
      if (data.success) {
        setMpPreferencia({ initPoint: data.init_point, comandaTotal: total });
        import('qrcode').then((QRCode) => {
      ...
      setToast('❌ Error de conexión con Mercado Pago');
    ...
    <div className={className}>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={mpLoading}
          className="flex-1 flex items-center justify-center gap-2 bg-[#00B9FF] text-white px-4 py-2.5 rounded-xl hover:bg-[#009EE0] transition-colors text-sm font-semibold disabled:opacity-50"
        >
          {mpLoading ? (
      ...
      {mpPreferencia && (
        ...
        >
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl p-6 text-center animate-in zoom-in-95 duration-200">
            ...
            <div className="mb-1">
              <p className="text-xs text-gray-400 mb-1">Escanea el código QR con tu celular</p>
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={async () => {
                  setMpChecking(true);
                  try {
                    const res = await fetch(`/api/mercadopago/check-payment?external_reference=${comandaId || ''}`);
                    const data = await res.json();
                    if (data.paid) {
                      setToast('✅ Pago aprobado con Mercado Pago');
                      setMpPreferencia(null);
                      onSuccess?.();
                    } else {
                      setToast('⏳ El pago aún no se ha completado');
                    }
                  } catch { setToast('❌ Error al verificar pago'); }
                  setMpChecking(false);
                }}
                disabled={mpChecking}
