...
export async function GET(request: Request) {
  try {
    ...
    let sql = `
      SELECT c.id, c.mesa_nombre AS mesa, c.mozo_id, c.mozo_nombre,
             c.estado, c.metodo_pago, c.total, c.fecha, c.hora, c.notas,
             JSON_ARRAYAGG(
               JSON_OBJECT(
...
export async function POST(request: Request) {
  try {
    ...
    const total = items.reduce((s: number, i: any) => s + Number(i.precio) * Number(i.cantidad), 0);
    ...
    const comida   = items.filter((i: any) => i.categoria === 'comida').reduce((s: number, i: any) => s + Number(i.precio) * Number(i.cantidad), 0);
    const bebidas  = items.filter((i: any) => i.categoria === 'bebidas').reduce((s: number, i: any) => s + Number(i.precio) * Number(i.cantidad), 0);
    await pool.query(
