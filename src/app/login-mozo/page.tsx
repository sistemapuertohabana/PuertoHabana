...
export default function LoginMozoPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    try {
      ...
      const user = personal.find((p: any) => p.email === email.trim() && p.rol === 'mozo');
      
      if (user) {
        localStorage.setItem('ph_mozo_session', JSON.stringify(user));
        router.push('/mozo');
      } else {
        setError('No se encontró un mozo con este correo.');
      }
    } catch {
