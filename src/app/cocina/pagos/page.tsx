...
export default function PagosCocinaPage() {
  ...
  const fetchNotifs = useCallback(async () => {
    if (!session) return;
    try {
      const params = new URLSearchParams({ rol_destino: 'cocina' });
  ...
  const marcarTodasLeidas = async () => {
    await Promise.all(notifs.filter(n => !n.leida).map(n => marcarLeida(n.id)));
  };
        ...
        <div className="space-y-3">
          {notifs.length > 0 && noLeidas > 0 && (
            <button onClick={marcarTodasLeidas}
              className="flex items-center gap-2 text-xs text-orange-600 hover:text-orange-700 font-medium ml-auto">
          ...
          {notifs.length === 0 ? (
            ...
            notifs.map(n => (
              <div key={n.id}
                className={`bg-white rounded-2xl border p-4 transition-all ${n.leida ? 'border-gray-100 opacity-70' : 'border-orange-200 shadow-sm'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${n.leida ? 'bg-gray-100 text-gray-400' : 'bg-orange-100 text-orange-600'}`}>
                      <Bell size={15} />
                    </div>
                    <div>
                  ...
                  {!n.leida && (
                    <button onClick={() => marcarLeida(n.id)}
                      className="shrink-0 text-xs text-orange-500 hover:text-orange-700 font-medium px-2 py-1 rounded-lg hover:bg-orange-50 transition-colors">
                      Leído
                    </button>
                  )}
                </div>
